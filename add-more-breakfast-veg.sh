...
# 备份文件
cp random-food.html random-food.html.backup-more-breakfast-veg
...
# 为每个食物添加条目
for i in "${!breakfast_veg_foods[@]}"; do
    food_name="${breakfast_veg_foods[$i]}"
    calories=$((200 + i * 5))
    protein=$((8 + i))
    carbs=$((25 + i * 2))
    fat=$((5 + i))
...
echo "完成！现在Breakfast Vegetarian有${#breakfast_veg_foods[@]}个选择了！" 